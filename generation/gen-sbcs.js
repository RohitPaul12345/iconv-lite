...
let destFileName = "encodings/sbcs-data-generated.js";


let encodingFamilies = [
    {
...
encodingFamilies.forEach(function(family){
    family.encodings.forEach(function(encoding){
        ...
        let encodingIconvName = encoding.name ? encoding.name : encoding; 
        let encodingName = encodingIconvName.replace(/[-_]/g, "").toLowerCase();
...
function generateCharsString(encoding) {
    ...
    let iconvToUtf8 = new Iconv(encoding, "UTF-8");
    let iconvFromUtf8 = new Iconv("UTF-8", encoding);
    let chars = "", needReverse = false, containsDiacritics = [];

    for (var b = 0x0; b < 0x100; b++) {
        try {
            let convertedChar = iconvToUtf8.convert(Buffer.from([b])).toString();
            
            if (convertedChar.length != 1)
                throw new Error("Single-byte encoding error: Must return single char.");

            let convertedBackBuf = iconvFromUtf8.convert(Buffer.from(convertedChar));
    ...
    let asciiString = '\x00\x01\x02\x03\x04\x05\x06\x07\x08\t\n\x0b\x0c\r\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f'+
